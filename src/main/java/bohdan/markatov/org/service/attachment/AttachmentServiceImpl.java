...
public class AttachmentServiceImpl implements AttachmentService {
    ...
    @Override
    public AttachmentResponseDto save(User user, Long taskId, String filePath) {
        if (attachmentRepository.existsByTaskIdAndFileName(taskId, getFileName(filePath))) {
            throw new NotUniqueValueException("File name must be unique");
        }
        Task task = taskService.get(user, taskId);
        Attachment attachment = saveNewAttachment(filePath, task);
        AttachmentResponseDto responseDto = attachmentMapper.toDto(attachment);
        responseDto.setFilePublicLink(dropboxService
                .getFilePublicLink(attachment.getDropboxFileId()));
        return responseDto;
    }
    ...
    private Attachment saveNewAttachment(String filePath, Task task) {
        Attachment attachment = new Attachment();
        attachment.setTask(task);
        attachment.setDropboxFileId(dropboxService.uploadFile(filePath, task.getId()));
